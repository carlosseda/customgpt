...
class Table extends HTMLElement {
  ...
  async getTableData () {
    ...
    this.data.forEach(element => {
      ...
      Object.keys(recordButtons).forEach((recordButton) => {
        ...
        if (recordButtons[recordButton] === 'scrapping') {
          tableButton.classList.add('scrapping-button')
          tableButton.dataset.id = element.id
          tableButton.innerHTML = `
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
  ...
  async renderTableButtons () {
    ...
    tableSection.addEventListener('click', async (event) => {
      ...
      if (event.target.closest('.scrapping-button')) {
        const editButton = event.target.closest('.scrapping-button')
        const endpoint = import.meta.env.VITE_API_URL + this.getAttribute('endpoint') + '/scrapping/' + editButton.dataset.id
        ...
        try {
          ...
          if (response.status === 200) {
            document.dispatchEvent(new CustomEvent('message', {
              detail: {
                message: 'Scrapping realizado correctamente',
                type: 'success'
              }
            }))
          }
